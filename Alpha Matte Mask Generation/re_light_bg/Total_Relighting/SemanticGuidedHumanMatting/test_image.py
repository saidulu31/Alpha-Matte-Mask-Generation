...
import utils
from utils.evaluate import MetricMAD, MetricMSE, MetricGRAD, MetricCONN, MetricIOU
import inference
...
for i in range(num_image):
    ...
    pred_alpha_uint8 = (pred_alpha * 255).astype(np.uint8)

    # Display pred_alpha
    plt.imshow(pred_alpha_uint8, cmap='gray')
    plt.title('Predicted Alpha Mask')
    plt.axis('off')
    plt.show()
...
for i in range(num_image):
    image_path = image_list[i]
    image_name = os.path.basename(image_path)[:os.path.basename(image_path).rfind('.')]
    print(i, '/', num_image, image_name)
    ...
    # Save the predicted alpha mask to the specified output directory
